...
client.once('ready', () => {
	console.log(chalk.magenta('MOCHI ADMIN'), `I have logged in as ${client.user.tag} (${client.user.id})`);
	client.user.setActivity(`${client.commandPrefix}help | ${process.env.OS_NAME}`);
});

client.on('guildMemberAdd', member => {
	// Send the message to a designated channel on a server:
	const channel = member.guild.channels.cache.find(ch => ch.name === 'mochi-logs');
  ...
  .setColor(`#${process.env.EMB_COLOR}`)
	...
	.addField("Information", stripIndents`**> Member:** ${member}
	**> Member ID:** ${member.id}
...
client.on('guildCreate', guild => {
  ...
  .setDescription(`Thanks for inviting me! My prefix is \`?\` or \`@${client.user.tag}\`.`)
  .addField('Discord.js Version', '12.2.0', true)
  .addField('Node Version', '12', true)
  .addField(`Developer of ${client.user.username}`, `Proudly developed by <@630817206145646602>!`)
